...
export const sendReminderWorkflow = serve(async (contex) => {
...
const fetchSubscriptionById = async (context, subscriptionId) => {
  return await context.run('fetchSubscription', () => {
    return Subscription.findById(subscriptionId).populate('user' , 'userId email');
  })
  };
...
const triggerReminder = async (context, lable ) => {
  ...
  console.log(`Triggering ${label} reminder for subscription ${context.requestPayload.subscriptionId}`);
